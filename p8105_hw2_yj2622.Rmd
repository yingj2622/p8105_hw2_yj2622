 ...
 * The total precipitation in 2018 was `r sum(pull(precip_df,total)[pull(precip_df,year) == 2018])` inches. The median    number of sports balls in 2017 was `r  median(pull(mr_trash_wheel_data,sports_balls)[pull(mr_trash_wheel_data,year) == 2017])`.
 ...
 * It contains information about station name, location, routes and whether it's an entry.
 ...
 * And these data are not tidy because how routes were arranged are not proper.
...
```{r n_ada_station}
n_station_with_ada <- select(nyc_transit_data,station_name,line,ada) %>% distinct(station_name,line,.keep_all = TRUE) %>% 
  filter(ada == TRUE) %>%
  nrow
```
...
Next, I read and clean the data in snp.csv in a similiar way :
...
Read the first ten lines of snp_data:
...
Now let's take a look at the first ten rows of cleaned dataset:
...
`r ncol(pols_month_data)` columns, and the range of year is `r min(pull(pols_month_data,year))` to `r max(pull(pols_month_data,year))`. It provides information about year, month, number of governors, senators and representatives who are democratic or republican respectively, and the president's party at that time.

* In the snp dataset, there are `r nrow(snp_data)` rows and `r ncol(snp_data)` columns. The year ranges from `r min(pull(snp_data,year))` to`r max(pull(snp_data,year))`. It provides information about time and the closing values of the S&P stock index.
...
* Finally, the merged dataset have`r nrow(pol_snp_unemp_data)` rows and`r ncol(pol_snp_unemp_data)` columns. The year ranges from `r min(pull(pol_snp_unemp_data,year))` to`r max(pull(pol_snp_unemp_data,year))`. It includes all information in the three datasets.